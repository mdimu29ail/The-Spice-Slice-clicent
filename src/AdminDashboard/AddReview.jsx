...
const AddReview = () => {
  ...
  const [isUploading, setIsUploading] = useState(false);
  const [imagePreview, setImagePreview] = useState(null);
  const [imageUrl, setImageUrl] = useState(''); // এটি খালি থাকলেও এখন কাজ করবে
  ...
  const handleImageUpload = async e => {
    ...
    try {
      // বাকেট নাম 'avatars' নিশ্চিত করুন (অথবা আপনার বাকেটের নাম দিন)
      const { error: uploadError } = await supabase.storage
        .from('avatars')
        .upload(filePath, file);
      ...
      Swal.fire(
        'Upload Failed',
        'Check if bucket "avatars" is public.',
        'error',
      );
      setImagePreview(null);
    } finally {
  ...
  const handleSubmit = async e => {
    ...
    // যদি ইমেজ না থাকে, তবে একটি ডিফল্ট লাক্সারি অ্যাভাটার সেট হবে
    const finalImageUrl =
      imageUrl ||
      `https://api.dicebear.com/7.x/initials/svg?seed=${form.name.value}`;
    ...
    try {
      ...
      Swal.fire({
        title: 'Boutique Voice Published',
        text: 'The patron testimonial is now part of our heritage.',
        icon: 'success',
        background: '#fcf9f5',
        confirmButtonColor: '#1a1a1a',
        customClass: {
      ...
      Swal.fire('Publication Failed', err.message, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 lg:p-12 bg-[#fcf9f5] min-h-screen font-sans">
      <header className="mb-12">
        ...
        <p className="text-gray-400 font-bold uppercase tracking-[0.3em] text-[10px] mt-3 italic">
          Archiving artisanal testimonials
        </p>
      </header>
      ...
      <div className="max-w-5xl grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        >
          <div className="relative group mb-8">
            <div
              className={`w-48 h-48 rounded-[3.5rem] border-[10px] ${imageUrl ? 'border-green-500/20' : 'border-white/5'} overflow-hidden shadow-2xl relative transition-all duration-500`}
            >
              {imagePreview ? (
                <img
                  src={imagePreview}
          ...
          <h4 className="text-white font-black text-xl tracking-tighter uppercase italic leading-none">
            {imageUrl ? 'Identity Verified' : 'Patron Portrait'}
          </h4>
          <p className="text-gray-500 text-[9px] font-bold uppercase tracking-[0.4em] mt-3">
            Visual authenticity layer
          </p>
        </motion.div>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                ...
                <input
                  name="name"
                  required
                  className="w-full px-6 py-4 bg-[#fcf9f5] rounded-full outline-none font-bold text-sm shadow-inner text-[#1a1a1a]"
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-6 py-4 bg-[#fcf9f5] rounded-full outline-none font-bold text-sm shadow-inner text-[#1a1a1a]"
                  placeholder="e.g. Executive Foodie"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4 flex justify-between">
                <span>Rating</span>
                <span className="text-[#E65100]">0.0 - 5.0</span>
              </label>
              <input
                name="rating"
                type="number"
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows="4"
                className="w-full px-8 py-6 bg-[#fcf9f5] rounded-[2.5rem] outline-none font-bold italic text-gray-700 shadow-inner border border-transparent focus:border-orange-100 transition-all"
                placeholder="Describe the boutique experience..."
              />
            </div>
